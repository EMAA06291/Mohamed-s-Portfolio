...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      links.forEach((link) => {
        ...
        if (section && section.offsetTop <= scrollPosition) current = link.href;
      });
  ...
  return (
    <>
      <Navbar
        fluid
        className="!fixed !top-0 !left-0 !right-0 !z-[9999] !bg-[transparent] shadow-lg dark:!bg-[transparent]"
      >
        <NavbarBrand
        ...
        <NavbarToggle
          className="block lg:hidden transition-transform duration-500 ease-in-out"
